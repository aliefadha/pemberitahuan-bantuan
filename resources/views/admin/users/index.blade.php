<x-app-layout>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="px-4 sm:px-6 py-4 border-b border-gray-200 flex flex-col xl:flex-row xl:items-center xl:justify-between gap-4">
            <h3 class="text-lg font-semibold text-gray-800 shrink-0">Daftar User</h3>
            <div class="flex w-full flex-col sm:flex-row sm:items-center xl:w-auto gap-2">
                <form method="GET" action="{{ route('admin.users.index') }}" class="flex min-w-0 items-center gap-2 sm:flex-1">
                    ...
                    @if(request('search'))
                        <a href="{{ route('admin.users.index') }}" class="px-3 py-2 text-sm font-medium text-gray-600 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 transition">
                            Reset
                        </a>
                    @endif
                </form>
                <a href="{{ route('admin.users.create') }}" class="inline-flex items-center justify-center gap-1 px-4 py-2 bg-gray-900 text-white text-sm font-medium rounded-lg hover:bg-gray-800 transition shrink-0">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <table class="w-full text-sm text-left">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4">
                            @if($user->no_telepon)
                                <span class="text-green-600">{{ $user->no_telepon }}</span>
                            @else
                        ...
                        <td class="px-6 py-4">
                            <a href="{{ route('admin.users.edit', $user) }}" class="inline-flex items-center justify-center w-8 h-8 bg-yellow-100 text-yellow-700 rounded-lg hover:bg-yellow-200 transition">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
